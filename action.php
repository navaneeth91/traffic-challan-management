...
if (isset($_POST['client'])) {
    ...
    $phone=$_POST['pno'];
    $date=date("Y-m-d");
    $sql="INSERT INTO `u_info` (`uid`, `pid`, `name`, `adds`, `pno` ,`challandate`) 
    VALUES (NULL, '$pid', '$uname', '$adds', '$phone','$date')";
    $run=mysqli_query($con,$sql);
    if ($run) {
...
if (isset($_POST['vdetail'])) {
	...
	$pid=$_SESSION['pid'];
	$uid=$_SESSION['uid'];
	$sql="INSERT INTO `vehicle_detail` (`vid`, `uid`, `vhno`, `vtype`, `pid`, `ccost`, `cdesc`) 
	VALUES (NULL, '$uid', '$vno', '$vtype', '$pid', '0', '0')";
	$run=mysqli_query($con,$sql);
	if($run){
